...
@section('content')
    ...
    <div>
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          ...
          <li class="breadcrumb-item">
            <a href="{{ route('blocks.index', 'society_blocks') }}" class="text-dark opacity-75 text-decoration-none">Blocks</a>
          </li>
          <li class="breadcrumb-item">
            <a href="{{ route('blocks.view', $property->block->uuid) }}"
               class="text-dark opacity-75 text-decoration-none">Blocks details</a>
          </li>
  ...
  <span class="d-flex ms-4">
    <h5 class="mb-1 fw-semibold"> {{ ucfirst($property->type) }} #{{ ucfirst($property->property_no)  }}
      <span class="badge rounded bg-label-secondary small"> {{ ucfirst($property->category) }}</span>
      <span
        class="badge rounded bg-label-primary small "> {{ ucwords(str_replace('_', ' ', $property->const_status)) }}</span>
    </h5>
  </span>
  <h5 class="fw-semibold ms-4 my-0"> {{ucfirst($property->name)}}</h5>
  <p class="text-muted ms-4">{{ ucfirst($block->name) }}, {{ $property->address }}, <span
      class="fst-italic"> {{ $property->block->society->country }} </span>
  </p>
    ...
    <ul class="nav nav-pills flex-column flex-sm-row">
      <li class="nav-item">
        <button class="nav-link active w-100 text-start" data-bs-toggle="pill" data-bs-target="#info">
          <i class="ti tabler-home me-2 d-none d-sm-block"></i> Property Info
        </button>
      </li>

      <li class="nav-item">
        <button class="nav-link w-100 text-start" data-bs-toggle="pill" data-bs-target="#gallery">
          <i class="ti tabler-photo me-2 d-none d-sm-block"></i> Gallery
        </button>
      </li>

      @if($property->const_status !== "pending")
        <li class="nav-item">
          <button class="nav-link w-100 text-start" data-bs-toggle="pill" data-bs-target="#construction_details">
            <i class="ti tabler-building me-2 d-none d-sm-block"></i> Construction Details
          </button>
        </li>
      @endif
    </ul>
  ...
  <div class="tab-content mt-3 p-0 mt-5">
      ...
      <div class="card border-0 shadow-sm rounded-3 p-4 mb-5">
        <div class="d-flex justify-content-between">
          ...
          <span class="text-primary cursor-pointer"  data-bs-toggle="modal"
                data-bs-target="#edit_basic_property" title="Edit">  <i class="fa-solid fa-pen-to-square"></i></span>
        </div>
        <h5 class="fw-bold"> {{ucfirst($property->name)}}</h5>
        <div class="row g-3 mb-3">
          <div class="col-md-3 col-6">
            <div class="bg-light rounded-3 p-3"><p class="text-muted small text-uppercase fw-semibold mb-1 card_label">
                Property no</p>
          ...
          <div class="col-md-3 col-6">
            <div class="bg-light rounded-3 p-3"><p class="text-muted small text-uppercase fw-semibold mb-1 card_label">
                Status</p>
              <p
                class="fw-semibold text-dark  mb-0 text-primary">{{ ucwords(str_replace('_', ' ', $property->const_status)) }}</p>
            </div>
          </div>
          <div class="col-md-3 col-6">
            <div class="bg-light rounded-3 p-3"><p class="text-muted small text-uppercase fw-semibold mb-1 card_label">
                Block</p>
              <p class="fw-semibold text-dark  mb-0 text-primary">{{ $property->block->name }}</p></div>
          </div>
          <div class="col-md-3 col-6">
            <div class="bg-light rounded-3 p-3"><p class="text-muted small text-uppercase fw-semibold mb-1 card_label">
                Society</p>
              <p class="fw-semibold text-dark  mb-0 text-primary">{{ $property->block->society->name }}</p></div>
          </div>
        </div>
        <hr>
        ...
        <p class="text-muted small mb-0 ps-1"><i
            class="ti tabler-map-pin me-1 bg-dark"></i>{{ ucfirst($property->address)}}, <span
            class="fst-italic fw-semibold ">{{ ucfirst($property->block->society->city) }}</span>, <span
            class="fst-italic fw-semibold">{{ ucfirst($property->block->society->country) }} </span></p>
      </div>
              ...
              <div class="d-flex justify-content-between align-items-center mt-2">
                <div class="d-flex align-items-center gap-2 @if ($property->attachments->count() == 0) d-none @endif">
                  <div class="form-check d-flex align-items-center m-0">
                    ...
                    <label class="form-check-xs fw-semibold mb-0 cursor-pointer" for="select_all">Select All</label>
                  </div>
                </div>
                <span data-url="{{ route('societies.bulk_delete') }}" id="bulk_btn"
                          ...
                          <div class="flex-grow-1 d-flex align-items-center justify-content-between min-w-0">
                            ...
                            <div class="d-flex flex-column align-items-center gap-1 flex-shrink-0">
                              @cannot('edit_property')
                                @php $ext = pathinfo($attachment->link, PATHINFO_EXTENSION); @endphp
                                ...
                                @endif
                              @endcannot
                              <a href="{{ asset('storage/'.$attachment->link) }}" download
                                 class="btn btn-xs btn-icon btn-outline-success">
                                <i class="fa-solid fa-download fa-sm"></i>
                              </a>
                              @if (auth()->user()->can('edit_property'))
                                <a href="{{ route('attachment.delete', [$attachment->id]) }}"
        ...
        <div class="col-md-6">
          <div class="card border-0 shadow-sm rounded-3 p-4 h-100">
            <div class="d-flex justify-content-between align-items-center mb-3">
              ...
              <span class="text-primary cursor-pointer" data-bs-toggle="modal"
                    data-bs-target="#edit_dimensions_modal" title="Edit">
            ...
            @foreach($property->dimensions as $dimension)
              @php
                $converted_size = app()->make(\App\Http\Controllers\PropertiesController::class)->convertForUser($dimension->size, $dimension->unit);
              @endphp
              <div class="border border-secondary rounded mb-2 p-2">
                <div class="d-flex justify-content-between align-items-center">
                  <span class="fw-semibold">{{ ucfirst($dimension->name) }}</span>
                  <div class="d-flex align-items-center gap-1">
                    <span class="fw-semibold">{{ round($converted_size, 2) }}</span>
                    <span class="badge bg-label-secondary">{{ ucfirst($dimension->unit) }}</span>
                  </div>
                </div>
              </div>
            @endforeach
            ...
            @php
              $area_sqft = GetArea::calculate($property->dimensions);
              $area = ""; $unit = "";
              if ($area_sqft >= $default_marla_size) {
                $in_marla = $area_sqft / $default_marla_size;
                if ($in_marla >= 20) { $area = $in_marla / 20; $unit = $area > 1 ? "Kanals" : "Kanal"; }
                else   { $area = $in_marla;       $unit = $in_marla > 1 ? "Marlas" : "Marla"; }
              } else { $area = $area_sqft; $unit = "Square Feet"; }
            @endphp
            ...
            <div class="border border-secondary rounded mb-2 p-2">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-1">
                  <span class="fw-semibold">{{ round($area, 2) }}</span>
                  <span class="badge bg-label-secondary">{{ $unit }}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="tab-pane fade" id="construction_details" role="tabpanel">
      @if($property->const_status !== "pending" && $property->floors->count())
        ...
        <div class="row g-3 mb-4">
          <div class="col-4">
            <div class="card border-0 shadow-sm rounded-3 p-6 text-center">
              <div class="fs-4 fw-semibold">{{ $property->floors->count() }}</div>
              <div class="text-muted small">Floors</div>
            </div>
          </div>
          <div class="col-4">
            <div class="card border-0 shadow-sm rounded-3 p-6 text-center">
              <div class="fs-4 fw-semibold">{{ $totalUnits }}</div>
        ...
        @foreach($property->floors as $floor)
          <div class="card border-0 shadow-sm rounded-3 overflow-hidden mb-3">
            ...
            {{-- Floor header --}}
            <div class="floor-header-border bg-label-primary d-flex align-items-center justify-content-between flex-wrap gap-2 px-3 py-2">
              <div class="d-flex align-items-center gap-2">
                <div class="floor-icon-wrap d-flex align-items-center justify-content-center rounded-circle flex-shrink-0">
                  <i class="ti tabler-building"></i>
                </div>
                <div>
                  <div class="floor-name fw-semibold">{{ ucfirst($floor->floor_type) }}</div>
                ...
                @foreach($floor->units as $unit)
                  <div class="border rounded-3 overflow-hidden mb-3">
                    <div class="unit-header-bg d-flex align-items-center flex-wrap gap-2 px-3 py-2">
                      <div class="unit-icon-wrap d-flex align-items-center justify-content-center rounded-circle flex-shrink-0">
                        <i class="ti tabler-home"></i>
                      </div>
                      <span class="unit-name fw-semibold">
                    Unit {{ $unit->unit_no }}{{ $unit->unit_name ? ' — '.ucfirst($unit->unit_name) : '' }}
                  </span>
                      @if($unit->unit_type)
                        <span class="badge rounded-pill unit-type-pill ">{{ ucfirst($unit->unit_type) }}</span>
                      @endif
...
@push('styles')
  <style>
    ...
    .floor-header-border { border-bottom:1px solid #b5aeff; }
    .floor-icon-wrap { width:30px; height:30px; background:#B5D4F4; color:#0C447C; font-size:.85rem; }
    .floor-name      { font-size:.85rem; color:#0C447C; }
    .floor-meta      { font-size:.68rem; color:#185FA5; letter-spacing:.03em; }

    .unit-header-bg  { background:#dfdfe3; border-bottom:1px solid #a4a4a4; }
    .unit-icon-wrap  { width:22px; height:22px; background: #b1b1b7a8; color:#27500A; font-size:.68rem; }
    .unit-name       { font-size:.75rem; color:#27500A; }
    .unit-type-pill  { background: #b1b1b7a8;   color:#27500A; font-size:.6rem; }

    /* Room header */
    .room-header-bg  { background:#c6dbef; border-bottom:1px solid #eaedf5; }
    .room-dot        { width:6px; height:6px; border-radius:50%; background:#5477a1; flex-shrink:0; }
    .room-name       { font-size:.75rem; }

    /* Dimension chips */
    .dim-side-chip   { background:#f1f3f5; border:0.5px solid #dee2e6; font-size:.68rem; }
    .dim-side-name   { font-size:.68rem; font-weight:600; color:#212529; }
    .dim-side-unit   { background:#c8cfe4; color:#6c757d; font-size:.6rem; border-radius:3px; padding:1px 4px; }

    .area-chip       { background:#E6F1FB; border:0.5px solid #85B7EB; font-size:.68rem; }
    .area-val        { font-weight:600; color:#0C447C; }
    .area-unit       { font-size:.6rem; color:#185FA5; }

    .amenity-chip    { background:#f8f9fc; border:0.5px solid #dee2e6; font-size:.65rem; color:#6c757d; }

    .btn-del-labeled { font-size:.68rem; padding:4px 10px; border:0.5px solid #f09595; color:#a32d2d; border-radius:6px; white-space:nowrap; }
    .btn-del-labeled:hover { background:#FCEBEB; color:#a32d2d; }
    .btn-del-icon    { font-size:.54rem; padding:3px 7px; border:0.5px solid #f09595; color:#a32d2d; border-radius:5px; }
    .btn-del-icon:hover { background:#FCEBEB; color:#a32d2d; }

    .section-sublabel { font-size:.62rem; font-weight:500; text-transform:uppercase; letter-spacing:.06em; color:#6c757d; }
    .room-grid        { display:grid; grid-template-columns:repeat(auto-fit, minmax(200px,1fr)); gap:.5rem; }

    .chevron    { display:inline-block; width:10px; height:10px; border-right:1.5px solid currentColor; border-bottom:1.5px solid currentColor; transform:rotate(45deg); margin-top:-3px; transition:transform .2s; }
    .chevron.up { transform:rotate(-135deg); margin-top:2px; color: #02488f;}
  </style>
@endpush
...
@push('scripts')
  <script>
    ...
    document.getElementById('edit_basic_property').addEventListener('show.bs.modal', () =>
      ...
      // Edit construction modal — same logic as create page create_property.js
      let editIsCommercial = {{ $property->category === 'commercial' ? 'true' : 'false' }};
